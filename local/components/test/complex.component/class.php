...
class SobaDirectionsComplex extends \CBitrixComponent
{
	/**
	...
	protected $arDefaultVariableAliases404 = array("detail" => array(
        "ELEMENT_ID" => "ID"
    ));
	...
	private function setPageParams()
	{
		...
		switch ($this->arResult['COMPONENT_PAGE']) {
            ...
            case 'section': {
                $APPLICATION->AddChainItem($curPageName, $this->arUrlTemplatesReplaced['section']);
                break;
            }
			case 'detail': {
				$APPLICATION->AddChainItem($curPageName, $this->arUrlTemplatesReplaced['detail']);
				break;
			}
			case 'add': {
				$APPLICATION->AddChainItem($curPageName, $this->arUrlTemplatesReplaced['add']);
				break;
			}
		}
	}
	...
	protected function getResult()
	{
		...
		if (!$this->arResult['COMPONENT_PAGE']) {
			$this->arResult['COMPONENT_PAGE'] = "list";
			$b404 = true;
		}
